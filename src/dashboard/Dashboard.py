...
with st.sidebar:
    ...
    exchanges = ["binance", "kraken", "coinbase", "bybit", "kucoin"]
    current_exchange = settings.get("active_exchange", "binance")
    ...
    if selected_exchange != current_exchange:
        settings["active_exchange"] = selected_exchange
    ...
    with st.expander("LLM Configuration"):
        # Load current settings again in case they were updated above
        settings = ConfigManager.load_settings()
        settings = ConfigManager.load_settings()
...
with col1:
    ...
    def render_live_chart():
        df = live_cache.get_data(active_exchange, st.session_state['current_symbol'])
        if df is not None and not df.empty:
            df = MarketDataProcessor.add_indicators(df)
            from streamlit_lightweight_charts import renderLightweightCharts
            charts = render_candlestick_chart(df, title=st.session_state['current_symbol'])
            renderLightweightCharts(charts, key="live_chart")
        else:
            with st.spinner("Connecting to live websocket stream..."):
                st.info("Market data syncing from exchange... Please wait.")
...
with col2:
    ...
    if st.button("Run Fresh Analysis", width="stretch"):
